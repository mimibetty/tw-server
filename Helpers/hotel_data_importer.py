...
def process_hotel_data(hotel: Dict[str, Any], postal_code: str) -> Dict[str, Any]:
    ...
    if isinstance(hotel_data['ratingHistogram'], list) and len(hotel_data['ratingHistogram']) == 5:
...
def insert_hotel_via_api(hotel_data: Dict[str, Any], api_url: str, token: str) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            logger.error(f"Failed to insert hotel: {hotel_data['name']} - Status: {response.status_code}")
...
def bulk_insert_hotels(postal_code: str, api_url: str, token: str, limit: int = 100, delay: float = 0.5) -> Dict[str, Any]:
    """
    ...
    for hotel in data:
        try:
            ...
            if not hotel_data['name'] or not hotel_data['latitude'] or not hotel_data['longitude']:
                errors.append(f"Missing required fields for hotel: {hotel_data.get('name', 'unknown')}")
                continue
