...
def process_thing_to_do_data(attraction: Dict[str, Any], postal_code: str) -> Dict[str, Any]:
    """
    ...
    if 'photos' in attraction and attraction['photos'] and len(attraction['photos']) > 30:
    ...
    if 'ratingHistogram' in attraction:
        ...
        elif isinstance(attraction['ratingHistogram'], list) and len(attraction['ratingHistogram']) == 5:
    ...
    if 'address' in attraction:
        if isinstance(attraction['address'], dict) and 'street' in attraction['address']:
    ...
    subcategories = []
    if 'subcategories' in attraction and isinstance(attraction['subcategories'], list):
    ...
    if isinstance(rating_histogram, list) and len(rating_histogram) == 5:
        ...
        if total > 0:
            calculated_rating = sum((i + 1) * rating_histogram[i] for i in range(5)) / total
...
def insert_thing_to_do_via_api(thing_to_do_data: Dict[str, Any], api_url: str, token: str) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        response = requests.post(api_url, json=thing_to_do_data, headers=headers)
        
        if response.status_code == 201:
            logger.info(f"Successfully inserted attraction: {thing_to_do_data['name']}")
            return response.json()
        else:
            logger.error(f"Failed to insert attraction: {thing_to_do_data['name']} - Status: {response.status_code}")
            logger.error(f"Response: {response.text}")
            return None
    except Exception as e:
        logger.error(f"Error inserting attraction {thing_to_do_data['name']}: {str(e)}")
        return None

def bulk_insert_things_to_do(postal_code: str, api_url: str, token: str, limit: int = 100, delay: float = 0.5, use_sample_data: bool = False) -> Dict[str, Any]:
    """
    ...
    for attraction in data:
        try:
            ...
            if not thing_to_do_data['name'] or not thing_to_do_data['latitude'] or not thing_to_do_data['longitude']:
                errors.append(f"Missing required fields for attraction: {thing_to_do_data.get('name', 'unknown')}")
                continue
            
            # Log the data we're about to insert (for debugging)
            logger.info(f"Processed attraction data: {json.dumps(thing_to_do_data, indent=2)}")
            
            # Insert the attraction via the API
            result = insert_thing_to_do_via_api(thing_to_do_data, api_url, token)
            
            if result:
                inserted += 1
            else:
                errors.append(f"Failed to create attraction: {thing_to_do_data['name']}")
            ...
            errors.append(f"{attraction.get('name', 'Unknown attraction')}: {str(e)}")
...
def main():
    ...
    try:
        ...
        limit = 5  # Default number of attractions to import if env var is invalid
        
    api_url = 'http://127.0.0.1:8000/api/things-to-do/'  # URL of the things-to-do API endpoint
    use_sample_data = False  # Use API data by default, set to True only for testing
    ...
    logger.info(f"Importing up to {limit} attractions (set via IMPORT_LIMIT env var)")
    ...
    if result['success']:
        logger.info(f"Successfully imported {result['data']['inserted']} attractions")
