...
def process_restaurant_data(
    restaurant: Dict[str, Any], postal_code: str
) -> Dict[str, Any]:
    """
    ...
    if (
        'photos' in restaurant
        and restaurant['photos']
        and len(restaurant['photos']) > 30
    ):
        restaurant['photos'] = restaurant['photos'][:30]

    # Process rating histogram - convert from object to list format
    if 'ratingHistogram' in restaurant and isinstance(
        restaurant['ratingHistogram'], dict
    ):
    ...
    if 'address' in restaurant:
        if (
            isinstance(restaurant['address'], dict)
            and 'street' in restaurant['address']
        ):
    ...
    if 'hours' in restaurant and restaurant['hours']:
        try:
            ...
            logger.warning(
                f'Error processing hours for {restaurant.get("name")}: {str(e)}'
            )
            hours = None
    ...
    if 'dietaryRestrictions' in restaurant and isinstance(
        restaurant['dietaryRestrictions'], list
    ):
    ...
    elif 'amenities' in restaurant and isinstance(
        restaurant['amenities'], list
    ):
    ...
    restaurant_data = {
        ...
        'menuWebUrl': restaurant.get('menuWebUrl')
    ...
    if (
        isinstance(restaurant_data.get('priceLevels'), list)
        and len(restaurant_data['priceLevels']) == 1
    ):
    ...
    if (
        isinstance(restaurant_data['ratingHistogram'], list)
        and len(restaurant_data['ratingHistogram']) == 5
    ):
...
def insert_restaurant_via_api(
    restaurant_data: Dict[str, Any], api_url: str, token: str
    ...
    try:
        ...
        if response.status_code == 201:
            logger.info(
                f'Successfully inserted restaurant: {restaurant_data["name"]}'
            )
            return response.json()
        else:
            logger.error(
                f'Failed to insert restaurant: {restaurant_data["name"]} - Status: {response.status_code}'
            )
            logger.error(f'Response: {response.text}')
            return None
    except Exception as e:
        logger.error(
            f'Error inserting restaurant {restaurant_data["name"]}: {str(e)}'
        )
        return None
...
def bulk_insert_restaurants(
    ...
    for restaurant in data:
        try:
            ...
            if (
                not restaurant_data['name']
                or not restaurant_data['latitude']
                or not restaurant_data['longitude']
                or not restaurant_data.get('image')
            ):
                errors.append(
                    f'Missing required fields for restaurant: {restaurant_data.get("name", "unknown")}'
                )
                continue
            ...
            if result:
                ...
                errors.append(
                    f'Failed to create restaurant: {restaurant_data["name"]}'
                )
            ...
            errors.append(
                f'{restaurant.get("name", "Unknown restaurant")}: {str(e)}'
            )
...
def main():
    ...
    try:
        ...
        limit = (
            5  # Default number of restaurants to import if env var is invalid
        )
    ...
    logger.info(
        f'Importing up to {limit} restaurants (set via IMPORT_LIMIT env var)'
    )
    ...
    if result['success']:
        logger.info(
            f'Successfully imported {result["data"]["inserted"]} restaurants'
        )
