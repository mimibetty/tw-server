...
def benchmark_all_methods(points, iterations=1000):
    ...
    for i, j, name in test_pairs:
        ...
        print(
            f'  Chênh lệch H vs Geo: {abs(h_dist - geo_dist):.2f} km ({abs(h_dist - geo_dist) / geo_dist * 100:.2f}%)'
        )

    # Thống kê tổng thể
    print('\n=== OVERALL STATISTICS ===')
    haversine_vs_geodesic = [
        abs(h - g) for h, g in zip(haversine_distances, geodesic_distances)
    ]
    great_circle_vs_geodesic = [
        abs(gc - g)
        for gc, g in zip(great_circle_distances, geodesic_distances)
    ]

    print(
        f'Chênh lệch trung bình Haversine vs Geodesic: {sum(haversine_vs_geodesic) / len(haversine_vs_geodesic):.2f} km'
    )
    print(
        f'Chênh lệch tối đa Haversine vs Geodesic: {max(haversine_vs_geodesic):.2f} km'
    )
    print(
        f'Chênh lệch trung bình Great Circle vs Geodesic: {sum(great_circle_vs_geodesic) / len(great_circle_vs_geodesic):.2f} km'
    )
...
# Chạy benchmark
results, h_distances, gc_distances, geo_distances = benchmark_all_methods(
    test_points_15, iterations=50
)
