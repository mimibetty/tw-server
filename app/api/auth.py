...
@bp.post('/sign-in/')
def sign_in():
    inputs = SignInSchema().load(request.get_json())
...
@bp.post('/sign-up')
def sign_up():
    inputs = SignUpSchema().load(request.get_json())
...
@bp.post('/send-otp')
def send_otp():
    inputs = SendOTPSchema().load(request.get_json())
...
def forgot_password():
    inputs = ForgotPasswordSchema().load(request.get_json())
